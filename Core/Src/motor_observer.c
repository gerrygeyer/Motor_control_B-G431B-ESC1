...
static void sliding_mode_observer(FOC_HandleTypeDef *pHandle_foc){
    ...
    alphabeta_sigmoid.alpha = sigmoid_q10(CLAMP_INT32_TO_INT16((int32_t)sliding_mode.alpha_gain * ((int32_t)sliding_mode.I_est.alpha - pHandle_foc->I_alph_bet_q15.alpha)));
    alphabeta_sigmoid.beta = sigmoid_q10(CLAMP_INT32_TO_INT16((int32_t)sliding_mode.alpha_gain * ((int32_t)sliding_mode.I_est.beta - pHandle_foc->I_alph_bet_q15.beta)));

    sliding_mode.kH.alpha = (alphabeta_sigmoid.alpha * (int32_t)sliding_mode.k_gain);
    sliding_mode.kH.beta = (alphabeta_sigmoid.beta * (int32_t)sliding_mode.k_gain);
